...
// =================================================================
// KÄYNNISTYSLOGIIKKA: ODOTETAAN, ETTÄ SIVU ON VALMIS
// =================================================================
document.addEventListener('DOMContentLoaded', () => {
    // Asetetaan "tarkkailija", joka etsii työkalun elementtejä.
    const maxAttempts = 50; // Yritetään enintään 5 sekunnin ajan (50 * 100ms)
    let attempts = 0;

    const toolWatcher = setInterval(() => {
        const phaseTitle = document.getElementById('phase-title');
        attempts++;

        // Jos phase-title löytyy, sivu on valmis ja voimme käynnistää työkalun.
        if (phaseTitle) {
            console.log('GM-Työkalu löysi tarvittavat elementit ja käynnistyy.');
            clearInterval(toolWatcher); // Pysäytetään tarkkailija
            initializeGmTool(); // Käynnistetään varsinainen työkalu
        }
        // Jos elementtiä ei löydy tietyn ajan kuluessa, annetaan virheilmoitus.
        else if (attempts > maxAttempts) {
            clearInterval(toolWatcher);
            console.error('GM-Työkalu ei löytänyt tarvittavia elementtejä (esim. #phase-title) 5 sekunnin odotuksen jälkeenkään. Sivu ei ehkä latautunut oikein.');
        }
    }, 100); // Tarkistetaan tilanne 100 millisekunnin välein
});

// =================================================================
// VARSINAINEN TYÖKALUN KOODI
// Kaikki aiempi koodi on nyt tämän funktion sisällä.
// =================================================================
function initializeGmTool() {
    ...
    const unitTemplatesContainer = document.getElementById(
        "unit-templates-container"
    );
    ...
    const phases = [
        {
            name: "Liikevaihe",
            checklist: [
                "Siirrä Sinisen puolen yksiköt.",
                "Siirrä Punaisen puolen yksiköt sääntöjen ja doktriinin mukaisesti.",
                "Tarkista reaktiotuli ja varotoimenpiteet liikkeen aikana.",
            ],
        },
        {
            name: "Tiedusteluvaihe",
            checklist: [
                "Suorita Sinisen puolen tiedusteluheitot.",
                "Onnistuiko tiedustelu? Paljasta Punaisen yksiköitä ('?'-merkit) tarvittaessa.",
                "Suorita Punaisen puolen tiedustelu ja päivitä sodan sumua.",
            ],
        },
        {
            name: "Komentovaihe",
            checklist: [
                'Laske Punaisen KP-pooli: <input type="number" id="kp-calc-groups" min="0" style="width: 50px;"> Taisteluryhmää <button id="kp-calc-btn">Laske</button>',
                "Jaa Komentopisteet Sinisen puolen johtajille.",
                "Anna komennot ja aktivoi kyvyt (esim. Punaisen 'Aaltohyökkäys').",
                "Suunnittele ja ilmoita tulitukipyynnöt.",
            ],
        },
        {
            name: "Tulitoimintavaihe",
            checklist: [
                "Suorita kaikki ilmoitetut tulitoiminnot (suora tuli, epäsuora tuli).",
                "Heitä osumat ja vahingot.",
                "Päivitä osuman saaneiden yksiköiden tila (Vaurioitunut, Lamautunut, Tuhottu) Yksiköt-välilehdellä.",
            ],
        },
        {
            name: "Tilannevaihe",
            checklist: [
                "Suorita vaaditut Moraalitestit kaikille yksiköille, jotka kärsivät tappioita tai olivat lähellä tuhoutuneita yksiköitä.",
                "Tyhjennä moraalitestilista.",
                "Tarkista skenaarion tavoitteiden tilanne.",
                "Päivitä Voittopisteet (VP) molemmille osapuolille.",
                "Poista vuoron loppuun kestävät vaikutukset.",
            ],
        },
    ];
    ...
    const saveData = () => {
        ...
        // Listojen tallennus
        gmData.fireSupport = Array.from(
            document.querySelectorAll("#firesupport-list li")
        ).map((el) => ({
            desc: el.querySelector(".gm-unit-name").textContent,
            turn: el
                .querySelector("span:not(.gm-unit-name)")
                .textContent.replace("Saapuu vuorolla: ", ""),
        }));
        gmData.undetectedUnits = Array.from(
            document.querySelectorAll("#undetected-list li")
        ).map((el) => ({ desc: el.querySelector("span").textContent }));
        gmData.reinforcements = Array.from(
            document.querySelectorAll("#reinforcement-list li")
        ).map((el) => ({
            desc: el.querySelector(".gm-unit-name").textContent,
            turn: el
                .querySelector("span:not(.gm-unit-name)")
    ...
    const populateUnitTemplates = () => {
        ...
        unitTemplatesContainer
            .querySelectorAll(".template-add-btn")
            .forEach((btn) => {
    ...
    const addUnitToList = (event) => {
        ...
        if (!uniqueName) {
            showNotification(
                "Anna yksikölle nimi/tunnus ennen lisäämistä.",
                "warning"
            );
            nameInput.focus();
            return;
        ...
        const isNameTaken = gmData.activeUnits.some(
            (unit) => unit.name.toLowerCase() === uniqueName.toLowerCase()
        );
        if (isNameTaken) {
            showNotification(
                `Yksikkö nimellä "${uniqueName}" on jo olemassa. Anna yksilöllinen nimi.`,
                "warning"
            );
            nameInput.focus();
            return;
    ...
    const renderActiveUnits = () => {
        ...
        gmData.activeUnits.forEach((unitData) => {
            ...
            const abilitiesHtml = template.abilities
                .map(
                    (a) =>
                        `<li><strong>${a.name}${a.isDamagedEffect
                            ? ' <span class="vaurioitunut-tila">(Vaurioitunut)</span>'
                            : ""
                        }:</strong> ${a.description}</li>`
                )
                .join("");
            const armamentHtml = template.armament
                .map(
                    (w) =>
                        `<tr><td>${w.name}</td><td>${w.attack}</td><td>${w.damage}</td><td>${w.notes}</td></tr>`
                )
                .join("");
            ...
            cardDiv.innerHTML = `
                <div style="grid-column: 1 / -1; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                    <div>
                        <h4 style="margin:0; font-size: 1.2em;">${unitData.name
                } <span style="font-size:0.8em; color: #57534e;">(${template.name
                })</span></h4>
                        <span class="gm-unit-group">${unitData.groupId || "Ei ryhmää"
                }</span>
                    </div>
                ...
                <div class="unit-stat-grid" style="grid-column: 1 / -1;">
                     <div class="unit-stat"><div class="label">Suoja (S)</div><div class="value">${template.stats.s
                }</div></div>
                     <div class="unit-stat"><div class="label">Moraali (M)</div><div class="value">${template.stats.m
                }</div></div>
                     <div class="unit-stat"><div class="label">Taitotaso (TT)</div><div class="value">${template.stats.tt
                }</div></div>
                     <div class="unit-stat"><div class="label">Liike (L)</div><div class="value">${template.stats.l
                }</div></div>
                </div>
            ...
            cardDiv.querySelector(".tk-minus").addEventListener("click", () => {
                const unit = gmData.activeUnits.find(
                    (u) => u.instanceId === unitData.instanceId
                );
                if (unit && unit.tk > 0) {
            ...
            cardDiv
                .querySelector(".gm-unit-status-select")
                .addEventListener("change", (e) => {
                    const unit = gmData.activeUnits.find(
                        (u) => u.instanceId === unitData.instanceId
                    );
                    if (unit) {
                        unit.status = e.target.value;
                        saveData();
                    }
                });
            cardDiv
                .querySelector(".gm-delete-button")
                .addEventListener("click", () => {
                    gmData.activeUnits = gmData.activeUnits.filter(
                        (u) => u.instanceId !== unitData.instanceId
                    );
                    renderActiveUnits();
                    saveData();
    ...
    const addToList = (type, desc, turn) => {
        ...
        switch (type) {
            case "firesupport":
                listEl = document.getElementById("firesupport-list");
                content = `<span class="gm-unit-name">${desc}</span><span>Saapuu vuorolla: ${turn}</span><button class="gm-delete-button">X</button>`;
                break;
            case "undetected":
                listEl = document.getElementById("undetected-list");
                content = `<span>${desc}</span><button class="reveal-button">Paljasta</button><button class="gm-delete-button">X</button>`;
                break;
            case "reinforcement":
                listEl = document.getElementById("reinforcement-list");
                content = `<span class="gm-unit-name">${desc}</span><span>Saapuu vuorolla: ${turn}</span><button class="gm-delete-button">X</button>`;
                break;
        }
        ...
        li.querySelector(".gm-delete-button").addEventListener("click", () => {
            li.remove();
            saveData();
        });
        if (type === "undetected")
            li.querySelector(".reveal-button").addEventListener("click", () => {
                showNotification(
                    `Yksikkö "${desc}" paljastettu. Lisää se manuaalisesti Yksiköt-välilehdeltä.`
                );
                li.remove();
                saveData();
            });
    ...
    const updatePhaseView = () => {
        // Turvatarkistukset heti alussa
        if (!phaseTitleEl || !phaseInstructionsEl || !phaseCounterEl || !prevPhaseBtn) {
            console.error("Yksi tai useampi vaihenäkymän kriittinen UI-elementti puuttuu HTML-tiedostosta. Tarkista ID:t: phase-title, phase-instructions, phase-counter, prev-phase-button.");
            return; // Pysäytetään funktion suoritus, jos elementtejä puuttuu
        }
        ...
        if (currentPhaseIndex === -1) {
            ...
            phase.checklist.forEach((item, index) => {
                const itemId = `phase${currentPhaseIndex}-item${index}`;
                checklistHtml += `<li><input type="checkbox" id="${itemId}"><label for="${itemId}">${item}</label></li>`;
            });
            checklistHtml += "</ul>";
            ...
            if (phase.name === "Komentovaihe") {
                document.getElementById('kp-calc-btn')?.addEventListener('click', () => {
                    const groupsInput = document.getElementById('kp-calc-groups');
                    const groups = parseInt(groupsInput.value) || 0;
                    const newKp = groups * 3;
                    // Lisätty turvatarkistus
                    if (kpPoolEl) {
                        kpPoolEl.textContent = newKp;
                    }
                    showNotification(`Komentopisteet päivitetty: ${newKp} KP`);
                    saveData();
                });
            } else if (phase.name === "Tulitoimintavaihe") {
                checkFireSupportArrival();
            } else if (phase.name === "Tilannevaihe") {
                const clearMoraleBtn = document.createElement('button');
                clearMoraleBtn.textContent = "Tyhjennä moraalitestilista";
                clearMoraleBtn.style.marginTop = "1rem";
                clearMoraleBtn.addEventListener('click', () => {
                    // Lisätty turvatarkistus
                    if (moraleCheckListEl) {
                        moraleCheckListEl.innerHTML = "<li>Ei yksiköitä, jotka vaativat testin.</li>";
                    }
                    saveData();
                });
                phaseInstructionsEl.appendChild(clearMoraleBtn);
            }
    ...
    checkFireSupportArrival = () => {
        const e = turnCounterEl.textContent,
            t = gmData.fireSupport || [];
        t.forEach((t) => {
            t.turn === e &&
                showNotification(`TULITUKI SAAPUU: ${t.desc}`, "warning");
        });
    },
        addUnitToMoraleCheck = (e) => {
            let t = !1;
            moraleCheckListEl.querySelectorAll("li").forEach((o) => {
                o.textContent === e && (t = !0);
            }),
                t ||
                (moraleCheckListEl.querySelectorAll("li").length === 1 &&
                    moraleCheckListEl
                        .querySelectorAll("li")[0]
                        .textContent.includes("Ei yksiköitä") &&
                    (moraleCheckListEl.innerHTML = ""),
                    (() => {
                        const t = document.createElement("li");
                        (t.textContent = e), moraleCheckListEl.appendChild(t);
                    })(),
                    showNotification(`${e} vaatii moraalitestin!`, "warning"),
                    saveData());
        };

    const updateTacticalAdvice = () => {
        if (!tacticalAdviceEl) return; // Varmistetaan, että elementti on olemassa

        const turn = parseInt(turnCounterEl.textContent);
        let advice = "";

        if (currentPhaseIndex === -1) {
            advice = "Peli odottaa alkamistaan. Valmistaudu ensimmäiseen siirtoon!";
        } else {
            const phaseName = phases[currentPhaseIndex].name;
            switch (phaseName) {
                case "Liikevaihe":
                    advice = "Hyödynnä maastoa. Siirrä tiedustelu kärkeen ja suojaa arvokkaammat yksiköt.";
                    break;
                case "Tiedusteluvaihe":
                    advice = "Paljastaako vastustaja asemiaan? Keskity havaitsemaan heikoin kohta.";
                    break;
                case "Komentovaihe":
                    advice = "Nyt jaetaan käskyt! Muista Punaisen puolen 4 KP:n käyttörajoitus per taisteluryhmä.";
                    break;
                case "Tulitoimintavaihe":
                    advice = "Keskity tuli yhteen kohteeseen tai käytä epäsuoraa tulta murtaaksesi linnoittautuneen vihollisen.";
                    break;
                case "Tilannevaihe":
                    if (moraleCheckListEl.children.length > 1 || moraleCheckListEl.children[0]?.textContent !== "Ei yksiköitä, jotka vaativat testin.") {
                        advice = "Moraali on avainasemassa! Varmista, että kaikki vaaditut testit on suoritettu.";
                    } else {
                        advice = "Tarkista tavoitteet ja pistetilanne. Suunnittele seuraavaa siirtoa.";
                    }
                    break;
                default:
                    advice = "Valitse seuraava toimenpide strategiasi mukaisesti.";
                    break;
            }
        }
        tacticalAdviceEl.textContent = advice;
    };
    ...
    prevPhaseBtn.addEventListener("click", () => {
        currentPhaseIndex === 0 && parseInt(turnCounterEl.textContent) > 1
            ? ((currentPhaseIndex = phases.length - 1),
                (turnCounterEl.textContent = parseInt(turnCounterEl.textContent) - 1))
            : currentPhaseIndex > 0 && currentPhaseIndex--,
            updatePhaseView();
    });
    const setupCounter = (e) => {
        const t = document.getElementById(`${e}-decrement`),
            o = document.getElementById(`${e}-increment`),
            a = document.getElementById(e);
        t.addEventListener("click", () => {
            let t = parseInt(a.textContent);
            t > 0 && (a.textContent = t - 1), saveData();
        }),
            o.addEventListener("click", () => {
                (a.textContent = parseInt(a.textContent) + 1), saveData();
            });
    };
    ["turn-counter", "kp-pool", "blue-vp", "red-vp"].forEach(setupCounter);
    const tabs = document.querySelectorAll(".gm-tab-button"),
        tabPanes = document.querySelectorAll(".gm-tab-pane");
    tabs.forEach((e) => {
        e.addEventListener("click", () => {
            tabs.forEach((e) => e.classList.remove("active")),
                tabPanes.forEach((e) => e.classList.remove("active")),
                e.classList.add("active"),
                document.getElementById(e.dataset.tab).classList.add("active");
        });
    });
    document
        .getElementById("add-firesupport-button")
        .addEventListener("click", () => {
            const e = document.getElementById("firesupport-desc-input"),
                t = document.getElementById("firesupport-turn-input");
            e.value.trim() &&
                t.value.trim() &&
                addToList("firesupport", e.value, t.value),
                (e.value = ""),
                (t.value = "");
        });
    document
        .getElementById("add-undetected-button")
        .addEventListener("click", () => {
            const e = document.getElementById("undetected-desc-input");
            e.value.trim() && addToList("undetected", e.value), (e.value = "");
        });
    document
        .getElementById("add-reinforcement-button")
        .addEventListener("click", () => {
            const e = document.getElementById("reinforcement-desc-input"),
                t = document.getElementById("reinforcement-turn-input");
            e.value.trim() &&
                t.value.trim() &&
                addToList("reinforcement", e.value, t.value),
                (e.value = ""),
                (t.value = "");
        });
    let notesTimeout;
    document.getElementById("gm-notes").addEventListener("keyup", () => {
        clearTimeout(notesTimeout);
        notesTimeout = setTimeout(saveData, 500);
    });
    secretNotesEl.addEventListener("keyup", () => {
        clearTimeout(notesTimeout);
        notesTimeout = setTimeout(saveData, 500);
    });
    const hideModal = () => {
